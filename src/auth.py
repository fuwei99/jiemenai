...
from .config import (
    ...
    CODE_ASSIST_ENDPOINT, GEMINI_AUTH_PASSWORD, get_next_credential, GEMINI_CREDENTIALS_LIST
)
...
def _load_creds_from_info(raw_creds_data: dict, is_from_env: bool):
    """Helper function to load Google Credentials from a dictionary."""
    global user_project_id, credentials_from_env

    source = "environment" if is_from_env else "file"
    ...
    if "refresh_token" in raw_creds_data and raw_creds_data["refresh_token"]:
        logging.info(f"Refresh token found in {source} credentials - ensuring credentials load successfully")
        ...
        try:
            ...
            if "expiry" in creds_data:
                ...
                if isinstance(expiry_str, str) and ("+00:00" in expiry_str or "Z" in expiry_str):
                    try:
                        ...
                        logging.info(f"Converted {source} expiry format from '{expiry_str}' to '{creds_data['expiry']}'")
                    except Exception as expiry_error:
                        logging.warning(f"Could not parse {source} expiry format '{expiry_str}': {expiry_error}, removing expiry field")
            ...
            credentials_from_env = is_from_env

            # Extract project_id from credentials if available
            if "project_id" in raw_creds_data:
                user_project_id = raw_creds_data["project_id"]
                logging.info(f"Extracted project_id from {source} credentials: {user_project_id}")

            # Try to refresh if expired and refresh token exists
            if creds.expired and creds.refresh_token:
                try:
                    logging.info(f"{source.capitalize()} credentials expired, attempting refresh...")
                    creds.refresh(GoogleAuthRequest())
                    logging.info(f"{source.capitalize()} credentials refreshed successfully")
                    if not is_from_env:
                        save_credentials(creds)
                except Exception as refresh_error:
                    logging.warning(f"Failed to refresh {source} credentials: {refresh_error}")
                    logging.info(f"Using existing {source} credentials despite refresh failure")
            elif not creds.expired:
                logging.info(f"{source.capitalize()} credentials are still valid, no refresh needed")
            elif not creds.refresh_token:
                logging.warning(f"{source.capitalize()} credentials expired but no refresh token available")
            
            return creds
            
        except Exception as parsing_error:
            # SAFEGUARD: Even if parsing fails, try to create minimal credentials with refresh token
            logging.warning(f"Failed to parse {source} credentials normally: {parsing_error}")
            logging.info(f"Attempting to create minimal {source} credentials with refresh token")
            ...
            try:
                ...
                creds = Credentials.from_authorized_user_info(minimal_creds_data, SCOPES)
                credentials_from_env = is_from_env
                
                # Extract project_id from credentials if available
                if "project_id" in raw_creds_data:
                    user_project_id = raw_creds_data["project_id"]
                    logging.info(f"Extracted project_id from minimal {source} credentials: {user_project_id}")
                
                # Force refresh since we don't have a valid token
                try:
                    logging.info(f"Refreshing minimal {source} credentials...")
                    creds.refresh(GoogleAuthRequest())
                    logging.info(f"Minimal {source} credentials refreshed successfully")
                    if not is_from_env:
                        save_credentials(creds)
                    return creds
                except Exception as refresh_error:
                    logging.error(f"Failed to refresh minimal {source} credentials: {refresh_error}")
                    # Even if refresh fails, return the credentials - they might still work
                    return creds
                    
            except Exception as minimal_error:
                logging.error(f"Failed to create minimal {source} credentials: {minimal_error}")

    else:
        logging.warning(f"No refresh token found in {source} credentials")
...
def get_credentials(allow_oauth_flow=True):
    ...
    # Priority 1: Use rotating credentials from environment if available
    if GEMINI_CREDENTIALS_LIST:
        creds_info = get_next_credential()
        if creds_info:
            loaded_creds = _load_creds_from_info(creds_info, is_from_env=True)
            if loaded_creds:
                # For rotating tokens, we don't cache globally. Return a fresh object.
                return loaded_creds
        logging.warning("Could not load a valid credential from GEMINI_CREDENTIALS. Falling back to other methods.")

    # Priority 2: Use cached credentials if not using rotating env creds
    if credentials and credentials.token:
        return credentials
    
    # Priority 3: Check for credentials file
    if os.path.exists(CREDENTIAL_FILE):
        try:
            with open(CREDENTIAL_FILE, "r") as f:
                raw_creds_data = json.load(f)
            
            loaded_creds = _load_creds_from_info(
                raw_creds_data, 
                is_from_env=bool(os.getenv("GOOGLE_APPLICATION_CREDENTIALS"))
            )
            if loaded_creds:
                credentials = loaded_creds  # Cache file-based creds
                return credentials
                
        except Exception as e:
            logging.error(f"Failed to read credentials file {CREDENTIAL_FILE}: {e}")

    # Priority 4: Start OAuth flow as last resort
